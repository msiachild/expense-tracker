...
def get_data(csv_url):
    try:
        ...
        # 3. 【修正重点】更精准的标题行判定
        # 我们检查第一行第一列是否包含具体的日期格式（比如数字/斜杠）
        # 如果第一行第一列看起来像文字标题（如 "date", "日期"），才删掉它
        first_cell = str(df_raw.iloc[0, 0]).lower()
        if "date" in first_cell or "日期" in first_cell or "时间" in first_cell:
            df_raw = df_raw.drop(index=0)

        # 4. 强制列名并清洗
        df_raw.columns = ["date", "category", "item", "amount"]
        
        # 强制转换 amount 为数字，报错的变 0
